...
class Numactl(Tool):
    ...
    def get_cxl_node(self) -> int:
        ...
        if len(match) == 0:
            raise SkippedException("CXL node not present")
        return match[0]

    def get_numa_nodes(self):
        output = self.help()
        match = self.__numa_nodes.findall(output.stdout)
        assert_that(len(match), "There should be atleast 1 NUMA node").is_greater_than(
            0
        )
        return len(match)

    def get_sysbench_memory_output(
        self,
        nodeId: int,
    ...
    def get_node_speed(self, output: ExecutableResult) -> float:
        match = self.__avg_speed.findall(output.stdout)
        return float(match[0])

    def get_node_time(self, output: ExecutableResult) -> float:
